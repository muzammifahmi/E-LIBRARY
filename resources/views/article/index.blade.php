...
<x-dynamic-component :component="$layout">
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($article->image)
                                                <img src="{{ asset('storage/articles/' . $article->image) }}"
                                                    alt="{{ $article->title }}" class="w-16 h-16 rounded object-cover">
                                            @else
    ...
    @guest
        {{-- Layout untuk pengunjung (belum login) --}}
        <div class="py-12 bg-gradient-to-b from-green-100 to-white min-h-screen font-sans">
            <div class="max-w-7xl mx-auto px-6 lg:px-8">
                <h2 class="text-3xl font-extrabold text-green-700 mb-8 text-center">Artikel Terbaru</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @forelse ($articles as $article)
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition">
                            ...
                            @endif
                            <div class="p-5">
                                <h3 class="font-bold text-lg text-green-800 mb-2 line-clamp-2">
                                    {{ $article->title }}
                                </h3>
                                <a href="{{ route('article.show', $article->id) }}"
                                    class="inline-block mt-3 px-4 py-2 bg-green-500 text-white rounded-full text-sm hover:bg-green-600">
                                    Baca Selengkapnya
                                </a>
                            </div>
                        </div>
